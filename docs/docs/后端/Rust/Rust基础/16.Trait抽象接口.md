...
```rust
trait TraitName {
	fn func_1(args...) -> ret;     // 关联函数

	fn func_2(&self, args..) -> ret;		// 方法
}
```

使用`trait`关键字，可以定义一个`Trait`。在它的名字后面，使用`{}`定义这个`Trait`所具有的方法。在定义时，往往只写出函数的签名，并且允许使用`Self`作为第一个参数，函数体暂时用`;`代替，表示还未实现。
...
使用`impl`关键字，可以为指定类型实现指定`Trait`，例如`impl Add for i32`就是给`i32`类型实现`Add`。在`{}`中，需要给每一个之前定义的方法做出实现，必须实现所有未提供默认实现的方法。
