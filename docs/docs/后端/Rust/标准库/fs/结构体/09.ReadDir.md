...
使用[`std::fs::read_dir`](../index#read-dir)函数读取目录时，返回的目录迭代器，遍历此迭代器，迭代器中每一项返回一个 `io::Result<DirEntry>` 实例。 通过 [`DirEntry`](./03.DirEntry)，可以了解类似条目路径以及可能的其他元数据的信息。
...
如果在迭代过程中出现某种间歇性的 IO 错误，则该 [`io::Result`](https://www.rustwiki.org.cn/zh-CN/std/io/type.Result.html) 将是 [`Err`](https://www.rustwiki.org.cn/zh-CN/std/result/enum.Result.html#variant.Err)。
