...
在绝大部分情况下，我们调用函数时不需要显式地去指定泛型的具体类型，因为`Rust`会根据上下文进行推导，主要是依据`参数`和`返回值`这两个区域进行推导。
...
`turbofish`是当我们调用泛型函数时，虽然rust会自动推导类型参数，但也有无法推导的时候，`turbofish`就是用来显示传入参数的语法
...
`turbofish`主要的使用场景在于一个无法推导的泛型函数中，如下示例
...
以上代码p没有显式的传入泛型，但是x和y默认类型为i32，所以rust会自动推导p是一个`Point<i32>`

而f我们给了显式泛型，所以x和y的类型必须是f64类型的
