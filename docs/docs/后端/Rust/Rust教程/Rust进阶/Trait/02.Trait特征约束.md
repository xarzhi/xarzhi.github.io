...
在进行泛型声明时，可以在对应的泛型后面使用`:`对这个泛型进行约束，多个类型使用`+`号连接

只有实现某些`Trait`的泛型，才能传入。泛型声明主要出现在函数、方法、复合类型、泛型`Trait`中，
...
比如有个面试函数，接收一个面试者，要求这个面试者必须实现Poilt这个trait
...
fn interview<T: Poilt + Driver>(h: T) {
    h.fly();
    h.drive();
}
...
前面我们已经知道，`println!()`宏对于有些数据类型，不可以直接打印，需要修改`{}`占位符

如果一个函数中的参数限定了某个泛型T，我想在函数中打印这个参数，由于编译器不知道这个T具体是什么类型，所以不会让你打印
...
impl<T> Point<T> {
    fn show(&self) {
        println!("x = {}, y = {}", self.x, self.y);  // 报错`T` doesn't implement `std::fmt::Display`
    }
}

fn main() {
    let p = Point { x: 1.11, y: 2.22 };
    p.show();
}

```

但是如果我的函数中的参数，我明确知道它是一个数值类型，我就想打印，怎么办呢

`std::fmt::Display`是标准库中一个`trait`，它用于控制`println!()`宏里占位符的数据是怎样打印的

我们给泛型定义上这么一个约束，这样我们就可以打印了
...
fn main() {
    let p = Point { x: 1.11, y: 2.22 };
    p.show();			// x = 1.11, y = 2.22
}
```

与`std::fmt::Display`类似的还有`std::fmt::Debug`通常用于打印复合数据类型
...
impl<T: std::fmt::Debug> MyStruct<T> {
    fn print_self(&self) {
        println!("{:#?}", self);
    }
}
