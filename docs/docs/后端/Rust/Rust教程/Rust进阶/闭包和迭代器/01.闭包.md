...
闭包时一种**匿名函数**，**它可以赋值给一个变量，也可以作为参数传递给其他函数**
...
`FnOnce`的核心特征是**只能被调用一次**，并且在该闭包内会拿走被捕获变量的所有权

捕获所有权意味着变量从原始作用域**移动**（move）到了闭包内部
...
`Fn` 特征以**不可变借用的**方式捕获环境中的值，一个闭包默认实现Fn特征
...
注意闭包作为参数时要使用泛型，以下代码中，有一个F泛型，泛型约束为`FnOnce(String)`，也就是说我们的闭包函数是`FnOnce`的，并且有一个`String`类型参数，然后在参数列表中给闭包变量设置这个F泛型
...
`move`关键字可以让一个闭包捕获变量时

- 对于非`Copy`类型，强制获取所有权，如`String`，`Vec`
- 对于实现`Copy`的类型，进行复制，如`i32`，`bool`
...
以上代码中的闭包默认是`Fn`类型的，我们再试试`FnOnce`
